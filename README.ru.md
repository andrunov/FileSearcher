# FileSearcher
[English](README.md)<br>
Настольное приложение для поиска файлов на локальных дисках. Это моё портфолио для демонстрации.
Работает как нативные поисковые системы ОС, но быстрее, имеет удобный интерфейс и возможность создания HTML-отчётов.
![FileSearcher](https://github.com/andrunov/myImages/blob/master/FileSearcher.JPG?raw=true)

## Что было сделано:
- алгоритм Левенштейна
- сортировка результатов по степени похожести с пейджингом
- умный поиск с учётом опечаток и грамматических вариаций (файлы с похожими названиями поднимаются выше, чего нет в стандартных поисковиках ОС)
- многопоточность (интерфейс был отделён от основной логики)
- алгоритмическая оптимизация (приложение работает действительно быстро)
- автоматическое масштабирование элементов в зависимости от размера окна
- кликабельные отчёты
- интернационализация

![FileSearcher](https://github.com/andrunov/myImages/blob/master/FileSearcherReport.JPG?raw=true)

## Download:
[Версия для Java 8](https://drive.google.com/file/d/1xZ8vCqG_YD0gCFbKaj1ZLNdrQJgUL6gR/view?usp=drive_link)<br>
[Версия для Java 11 и выше](https://drive.google.com/file/d/1xZ8vCqG_YD0gCFbKaj1ZLNdrQJgUL6gR/view?usp=drive_link)<br>
[Установить Java (если отсутствует)](https://www.java.com/en/download/manual.jsp)
